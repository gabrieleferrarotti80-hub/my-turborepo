// src/components/EditReportTecnicoForm.jsx
...
const EditReportTecnicoForm = ({ reportData, onClose }) => {
    // Stato per i dati del form
    const [formData, setFormData] = useState({
        ...reportData,
        // Inizializza la descrizione con il campo 'note'
        descrizione: reportData.note || '', 
    });
    const [isSaving, setIsSaving] = useState(false);

    // Dati globali per mappare gli ID ai nomi (non più necessari per il nome del tecnico/cantiere)
    const { users: personnel, cantieri: sites } = useFirebaseData();
    const { updateReport } = useReportsManager();

    // I nomi sono ora disponibili direttamente dai dati del report
    const authorName = reportData.tecnicoName || 'Non disponibile';
    const siteName = reportData.cantiereName || 'Non disponibile';
    
    // La URL della foto è ora disponibile direttamente come 'photoUrl'
    const imageUrl = reportData.photoUrl;

    // Converte il timestamp in un oggetto Date
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const dataToUpdate = {
                // 'tipologia' non è presente nei nuovi dati, ma la manteniamo per coerenza del form
                tipologia: formData.tipologia, 
                // Aggiorniamo il campo 'note' con il valore di 'descrizione'
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <p className="text-md text-gray-900">
                                {reportDate.toLocaleDateString('it-IT', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                            </p>
                        </div>
                        ...
                        {reportData.location?.latitude && (
                             <div>
                                ...
                                <a href={`https://www.google.com/maps/search/?api=1&query=${reportData.location.latitude},${reportData.location.longitude}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-indigo-600 hover:underline">
...
export default EditReportTecnicoForm;