...
export const AnalisiManodoperaView = ({ righeComputo = [], onBack, onConfirm, onSaveDraft }) => {
    ...
    return (
        <div className="bg-white rounded-3xl shadow-lg border border-slate-200 overflow-hidden mt-6 animate-fade-in relative flex flex-col h-[calc(100vh-140px)]">
            ...
            {/* 🌟 HEADER CON TASTO SALVA BOZZA IN EVIDENZA 🌟 */}
            <div className="bg-slate-900 p-6 flex flex-wrap justify-between items-center gap-4 shrink-0">
                ...
                <div className="flex items-center gap-4">
                    ...
                    {/* TASTO SALVA BOZZA IN ALTO */}
                    <button 
                        onClick={() => onSaveDraft(righeElaborate)} 
                        className="flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white px-5 py-3 rounded-xl font-bold transition-all shadow-md border border-slate-600 active:scale-95"
                    >
                        <CloudArrowUpIcon className="h-5 w-5 text-indigo-300" /> <span className="hidden sm:inline">Salva Bozza</span>
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white border-t border-slate-200 p-6 flex flex-wrap justify-between items-end gap-6 shrink-0 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.05)] z-10">
                ...
                <div className="flex gap-3 w-full lg:w-auto mt-4 lg:mt-0">
                    {/* Il tasto salva bozza lo teniamo anche qui sotto per comodità di chi arriva a fine pagina */}
                    <button 
                        onClick={() => onSaveDraft(righeElaborate)} 
                        className="bg-slate-700 hover:bg-slate-600 text-white px-6 py-4 rounded-2xl font-bold text-lg transition-all shadow-md active:scale-95 flex items-center justify-center gap-3 border border-slate-600 flex-1 lg:flex-none"
                    >
                        <CloudArrowUpIcon className="h-6 w-6 text-indigo-300" /> <span className="hidden sm:inline">Salva Bozza</span>
                    </button>
                    <button 
                        onClick={() => onConfirm(righeElaborate)} 
                        className="bg-slate-900 hover:bg-black text-white px-8 py-4 rounded-2xl font-black text-lg transition-all shadow-lg active:scale-95 flex items-center justify-center gap-3 flex-[2] lg:flex-none"
                    >
